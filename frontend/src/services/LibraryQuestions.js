...
class Service{
    ...
    async add({data, token}){
        ...
        const url = `${RUTA}/LibraryQuestions`
        let response = null
        ...
        await axios.post(url, data , {
          ...
          .then(function (resp) {
            ...
            response = resp
            toast.success("Opciones agregada en biblioteca", {
    ...
    async all({params, token}){ 
        ...
        try {
            ...
            this.data.value.forEach(item => {
              ...
              //   this.data.value =  [{"Id": "0", "Nombre":"NO HAY QUESTIONS"}]
              // }

            this.columns.value =  Object.keys(this.data.value[0]).filter(key => {
              // Aquí debes especificar las claves que deseas obtener
             return  key == 'Id'|| key == 'IdQuestion'|| key == 'TextoPregunta'||  key == 'IdDepartments' || key == 'UsrCrea' || key == 'FecCrea';
            });
          } catch (error) {
    ...
    async del({ params, token }) {
        ...
        const url = `${RUTA}/LibraryQuestions/delete`
        let response = null
    ...
    async unique({ params, token }) {
      const url = new URL(`${RUTA}/LibraryQuestions/unique`);
